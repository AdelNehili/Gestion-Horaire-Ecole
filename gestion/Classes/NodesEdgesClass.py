...
class _MatrixMaster:
    """
    Pour le moment, c'est cette classe qui crée une matrice (binaire) représentant le lien entre tous les slots de cours
        (histoire de commencer le projet sans devoir traduire une vraie DB. (voir README)
    Cette même matrice sera traduite en relation grâce à la fonction createRelationList().

    Enfin, son rôle est de créer une liste de couleur pour représenter les slots horaires.
    ...
    def createDefinedRelationMatrix(self, nbr_classes, randomizer = False):
        matrix = []
    ...
    def createRelationList(self, matrix):
        """
        Cette fonction traduit le besoin de networkx d'avoir une liste de tuples pour représenter les edges (graphTheory)
        (Voir constructor de la classe fille _G_Manager).
        De plus, cette fonction mise en parallèle avec create_Node_List() permet de créer la cohérance entre les nodes
        ,edges et la matrix.
            Ex: Node1 et Node2 sont liés dans la matrice alors ils doivent l'etre dans le graphDB.
        """
        relation_list = []
        for i in range(len(matrix)):
            for j, value in enumerate(matrix[i]):
                if value == 1:
                    #Si le cours i et j ont un lien
                    current_tuple = (i, j)
    ...
    def create_Node_List(self, nbr_nodes):
        """
        Cette fonction traduit le besoin de networkx d'avoir une liste pour représenter les nodes (graphTheory).
        De plus, cette fonction mise en parallèle avec createRelationList() permet de créer la cohérance entre les nodes
        ,edges et la matrix.
            Ex: Node1 et Node2 sont liés dans la matrice alors ils doivent l'etre dans le graphDB.
        """
        this_list = []
...
class _G_Manager(_MatrixMaster):
    """
    Cette classe a pour but de gérer l'entièreté du graph.
    Dans le cadre de ce projet, son rôle principale sera sort_according_to_degrees() qui return la liste nécessaire à
        l'algorithme de moindre couleur.

    Si besoin, elle peut ajouter des attributs aux nodes et aux edges.
    """
    def __init__(self, nbr_classes):
        super(_G_Manager, self).__init__()
        self.matrix = self.createDefinedRelationMatrix(nbr_classes, randomizer=True)
    ...
    def add_attributes_to_nodes(self, G):
        #Pour le moment, on ne rajoute pas d'attributs sur les nodes mais bon à garder
    ...
    def add_attributes_to_edges(self, G):
        #Pour le moment, on ne rajoute pas d'attributs sur les edges mais bon à garder
    ...
    def sort_according_to_degrees(self):
        ...
        Elle permet de créer une sorted_list en fonction du degree des nodes. C'est de cette liste que l'algo de moindre
        couleur sera appliqué.
        """
...
class MapManager(_G_Manager):
    ...
    def _nobody_got_current_color(self, color, color_map, current_node):
        """
        Fonction centrale.
        Cette fonction vérifie qu'aucun node voisin n'a déjà la couleur du current_node VOUDRAIT avoir.
        """
        res = 1
